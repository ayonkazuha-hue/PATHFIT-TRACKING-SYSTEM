...
CREATE TABLE health_screening (
    ...
    student_id        UUID NOT NULL UNIQUE REFERENCES users(user_id) ON DELETE CASCADE,
    injury_history    TEXT,
...
CREATE TABLE fitness_portfolio (
...
ALTER TABLE health_screening  ENABLE ROW LEVEL SECURITY;
ALTER TABLE fitness_portfolio ENABLE ROW LEVEL SECURITY;

-- users
CREATE POLICY "students_own_profile" ON users FOR SELECT USING (
    auth.uid() = user_id OR
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);
...
-- fitness_tests
CREATE POLICY "fitness_tests_select" ON fitness_tests FOR SELECT USING (
    student_id = auth.uid() OR
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);
CREATE POLICY "fitness_tests_insert" ON fitness_tests FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);
CREATE POLICY "fitness_tests_update" ON fitness_tests FOR UPDATE USING (
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);
...
CREATE POLICY "lesson_plans_write" ON lesson_plans FOR ALL USING (
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);

-- health_screening
CREATE POLICY "health_select" ON health_screening FOR SELECT USING (
    student_id = auth.uid() OR
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);
CREATE POLICY "health_insert" ON health_screening FOR INSERT WITH CHECK (student_id = auth.uid());
CREATE POLICY "health_update"  ON health_screening FOR UPDATE USING (
    student_id = auth.uid() OR
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);

-- fitness_portfolio
CREATE POLICY "portfolio_select" ON fitness_portfolio FOR SELECT USING (
    student_id = auth.uid() OR
    EXISTS (SELECT 1 FROM users u WHERE u.user_id = auth.uid() AND u.role = 'instructor')
);
CREATE POLICY "portfolio_insert" ON fitness_portfolio FOR INSERT WITH CHECK (student_id = auth.uid());
CREATE POLICY "portfolio_update" ON fitness_portfolio FOR UPDATE USING (student_id = auth.uid());

INSERT INTO lesson_plans (pathfit_level, week_number, topic, activity, objectives) VALUES
